...
const DailyRank = () => {
  ...
  return (
    <>
      <SmallHeader ref={headerRef} title='일간 차트' buttons={[button1to50, button51to100]} />
      <motion.div
      ...
      >
        <h1 className='text-gray-50 text-4xl font-bold'>일간 차트</h1>
          ...
          {ranks.ranking.map((item, index) => (
            <li key={item.videoIds[0]}>
              <ChartItem
                id={item.videoIds[0]}
                rank={index + 1}
                rankChange={chartData[item.artist][item.title].previousRank.daily - (index + 1)}
                title={item.title}
                artist={item.artist}
                count={item.count}
              />
